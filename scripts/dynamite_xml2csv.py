...
class xml2csv:
  ...
  def __init__(self, input_file, output_file, encoding='utf-8'):
    ...
    self.header_map = {
      ...
      'merchandisetype':'MerchantCategory',
      'standardshippingcost':'shipping',
      'name':'title',
      'imageurl':'image_link',
      'retailprice':'RegularPrice',
      'buyurl':'link',
      'advertisercategory':'google_product_category',
      'saleprice':'sale_price',
      'instock':'InStock',
      'keywords':'c:keywords:string'
      }
    self.output_buffer = []
    self.output = None
    ...
    for event, elem in self.context:
      ...
      if event == 'start':
        ...
        # end of traversing the record tag
          ...
          #re.sub('[\t+\n+]', '', s) to remove all the tabs and newlines
          self.output_buffer.append(r'"' + (r'"' + delimiter + r'"').join([re.sub('[\t+\n+]', ' ', each) for each in processed_fields.values()]) + r'"')
          processed_fields = dict(zip(self.header_line, [" "]*len(self.header_line)))
          n += 1
...
output_file = "/Users/PoojaNihalani/Downloads/dynamiteclothing_new.csv"
